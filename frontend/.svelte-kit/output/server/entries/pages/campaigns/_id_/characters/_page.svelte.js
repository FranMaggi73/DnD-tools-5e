...
const CharacterFormModal = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${isOpen ? ` <div class="modal modal-open z-50"><div class="bg-[#2d241c] text-base-content border-4 border-secondary corner-ornament card-parchment w-full max-w-2xl p-6 relative"> <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" data-svelte-h="svelte-weuy30">✕</button>  <h3 class="font-bold text-3xl font-medieval text-neutral mb-6 text-center">${escape(isEdit ? "✏️ Editar Personaje" : "🧙‍♂️ Crear Personaje")}</h3> <form class="space-y-4"> <div class="form-control"><label class="label mb-1" data-svelte-h="svelte-1pn00cu"><span class="label-text font-medieval text-neutral text-lg">Nombre *</span></label> <input type="text" placeholder="Ej: Gandalf el Gris" required class="input input-bordered bg-[#2d241c] text-base-content border-2 border-primary/50 focus:border-secondary focus:ring-0 focus:outline-none w-full transition-colors duration-150"${add_attribute("value", form.name, 0)}></div>  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div class="form-control"><label class="label mb-1" data-svelte-h="svelte-1hd80f6"><span class="label-text font-medieval text-neutral text-lg">HP Máximo *</span></label> <input type="number" min="1" required class="input input-bordered bg-[#2d241c] text-base-content border-2 border-primary/50 focus:border-secondary focus:ring-0 focus:outline-none w-full transition-colors duration-150"${add_attribute("value", form.maxHp, 0)}></div> <div class="form-control"><label class="label mb-1" data-svelte-h="svelte-1lykn8k"><span class="label-text font-medieval text-neutral text-lg">Clase de Armadura (CA) *</span></label> <input type="number" min="1" required class="input input-bordered bg-[#2d241c] text-base-content border-2 border-primary/50 focus:border-secondary focus:ring-0 focus:outline-none w-full transition-colors duration-150"${add_attribute("value", form.armorClass, 0)}></div></div>  <div class="modal-action justify-center gap-4 mt-4"><button type="button" class="btn btn-outline border-2 border-neutral text-neutral hover:bg-neutral hover:text-secondary font-medieval" data-svelte-h="svelte-hlh51s">Cancelar</button> <button type="submit" class="btn btn-dnd" ${!form.name || !form.maxHp ? "disabled" : ""}><span class="text-xl">${escape(isEdit ? "💾" : "✨")}</span> ${escape(isEdit ? "Guardar Cambios" : "Crear Personaje")}</button></div></form></div></div>` : ``}`;
});
